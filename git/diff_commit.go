...
func DiffCommit(dir string, ref string, compareRef string, options *model.DiffCommitOptions) (fileDetailsMap map[string]*model.FileDetail, err error) {
	...
	if ref != COMMIT_WORKING {
		defRef = fmt.Sprintf("ref=$(git rev-parse --verify --quiet %s^{commit} || true)", sh.Quote(ref))
	...
	res, err := RunCommand(dir, func(commands []string) []string {
		return append(commands, []string{
			defRef,
			fmt.Sprintf("compareRef=$(git rev-parse --verify --quiet %s^{commit} || true)", sh.Quote(compareRef)),
			// all new files
			"echo 'all-new-files:'",
